...
async function deploymentBuild() {
  console.log('Creating final deployment build with proper server structure...');
  ...
  try {
    ...
    // Build frontend using esbuild config
...
`;
    ...
    // Build backend using the new start.ts entry point
    console.log('Building backend server...');
    execSync('npx esbuild server/start.ts --platform=node --packages=external --bundle --format=esm --outfile=dist/server.js --target=node18', { 
      stdio: 'inherit'
    });
...
<html lang="en">
  ...
  <body>
    ...
    console.log('📦 Ready for deployment with proper server structure');
...
const success = await deploymentBuild();
