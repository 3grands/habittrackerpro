...
async function buildDeploymentFix() {
  console.log('Creating deployment build - fixing timeout issue...');
...
`;
    ...
    // Build backend using completely clean entry point
    console.log('Building backend with clean entry point...');
    execSync('npx esbuild server/start-clean.ts --platform=node --packages=external --bundle --format=esm --outfile=dist/server.js --target=node18', { 
      stdio: 'inherit'
    });
...
<html lang="en">
  ...
  <body>
    ...
    // Verify no cartographer references remain
    execSync('grep -q "cartographer\\|@replit/vite-plugin-cartographer" dist/server.js && echo "ERROR: Cartographer found in build" && exit 1 || echo "Clean build verified"', { 
      stdio: 'inherit',
      shell: true
    });
    ...
    console.log('Deployment timeout fix completed!');
    console.log('Frontend assets: dist/public/');
    console.log('Backend server: dist/server.js');
    console.log('Cartographer dependency completely eliminated');
...
const success = await buildDeploymentFix();
